...
- **No tunables on the Player blocks.** `PollInterval`, `SubTimeout`,
  ...
  to maintain.
- **121 unit tests** with a stubbed `Hsl3Framework`, runnable in CI.
...
├── tests/
│   └── test_logic_modules.py    121 unit tests with a stubbed framework
└── dist/
...
121 tests covering: pure helpers (SOAP fault extraction, NOTIFY parsing,
